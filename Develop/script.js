let apiKey =
  "83464db7448f440f2230d5eae25329a5";
const form =
...
function fetchCoordinates(
  ...
  let userInput =
    cityInputEl.value; //gets input value
    ...
    .then(function (data) {
      ...
      for (
        ...
        if (
          ...
          let dateText =
            arrayList[i]
              .dt_txt; /*.format('MMMM Do YYYY || h:mm a')*/
          let tempFahrenheit =
            arrayList[i].main
              .temp + " °F";
          let windSpeed =
...
function renderSearchHistory() {
  ...
  for (
    let i = 0;
    i < searcHistory.length;
    i++
  ) {
    var recentSearchesButton =
      document.createElement(
        "button"
      );
    var recentSearchesLi =
      document.createElement(
        "li"
      );
    recentSearchesLi.append(
      recentSearchesButton
    );
    recentSearchesButton.setAttribute(
      "data-search",
      searcHistory[i]
    );
    recentSearchesButton.textContent =
      searcHistory[i];
    recentSearchEl.append(
      recentSearchesLi
    );
  }
}
